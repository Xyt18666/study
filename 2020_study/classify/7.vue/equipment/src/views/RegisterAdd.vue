...
export default {
  data() {
    const checkNumber = (rule, value, callback) => {
      ...
      //     callback(new Error("设备数量必须为大于1的整数"));
      //   } else {
      //     callback();
      //   }
    };
    return {
      ...
      rules: {
        ...
        time: [{ required: true, message: "请选择购置日期", trigger: "blur" }]
      },
      categoryList: []
    };
  },
  methods: {
    save() {
      ...
      axios.post("/equipment/pre-add", this.formData).then(d => {
    ...
    getCategory() {
      axios.get("/equipment/category-list").then(({ data }) => {
        if (data.code === 0) {
